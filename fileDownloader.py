...
#Creates a new file and writes the content of downloadedFile[0] to it in the folder folderPath under the file
#name given by downloadedFile[1].
def writeDownloadedFile(downloadedFile,folderPath):
    fileData=downloadedFile[0].content
   
    if(os.path.exists(folderPath)==False):
        os.makedirs(folderPath) #Recursively creates the object folder if it doesn't already exist.

    fileName=downloadedFile[1]    
    writtenFilePath=folderPath+"/"+fileName
    writtenFile=open(writtenFilePath,"wb")
    writtenFile.write(fileData)
    writtenFile.close()
    ...
    if(downloadedFiles==None):
        ...
        for currentDownloadedFile in downloadedFiles:
            if(containsOkStatusCode(currentDownloadedFile[0])): #If the file can be downloaded.
                writeDownloadedFile(downloadedFile=currentDownloadedFile,folderPath=currentObjectFolderPath)
