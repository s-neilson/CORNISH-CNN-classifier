...
def trainBCNN(modelFileName,modelHistoryFileName,trainingDataGenerator,validationDataGenerator,trainingStepsPerEpoch,validationStepsPerEpoch,epochsPerHierarchyLevel,outputLayerNames,trainingLossWeight,earlyStoppingMinDelta,earlyStoppingPatience):
    ...
    for i,currentLossWeights in enumerate(lossWeights):
        ...
        earlyStoppingCallbackMonitorString="val_"+outputLayerNames[i]+"_loss" #The loss of the current model output being trained is monitored for early stopping.
        ...
        currentModelFit=loadedModel.fit_generator(generator=trainingDataGenerator,steps_per_epoch=trainingStepsPerEpoch,validation_data=validationDataGenerator,validation_steps=validationStepsPerEpoch,epochs=epochsPerHierarchyLevel,verbose=2,callbacks=[earlyStoppingCallback])
